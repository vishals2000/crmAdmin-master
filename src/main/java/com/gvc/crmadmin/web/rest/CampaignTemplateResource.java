...
public class CampaignTemplateResource {
    ...
    public ResponseEntity<PushNotificationCampaignTargetGroupContentSizeResponse> getPushNotificationTargetGroupSizeForLanguage(@Valid @RequestBody PushNotificationCampaignTargetGroupContentSizeRequest pushNotificationCampaignTargetGroupContentSizeRequest) throws URISyntaxException, UnsupportedEncodingException {
        ...
        pushNotificationCampaignTargetGroupSizeRequest.setRetargetedCampaign(pushNotificationCampaignTargetGroupContentSizeRequest.isRetargetedCampaign());
        pushNotificationCampaignTargetGroupSizeRequest.setParentCampaignTemplateId(pushNotificationCampaignTargetGroupContentSizeRequest.getParentCampaignTemplateId());
            ...
            if(campaignTemplate != null) {
                ...
                if(!campaignTemplate.isAlreadyCancelled()) {
                        ...
                        if (pushNotificationCampaignCancellationResponse.isResult()) {
                            ...
                            if(!campaignTemplate.getRecurrenceType().equals(RecurrenceType.NONE) && currentTime.isAfter(campaignStartTime)) {
    ...
    */
    ...
    @PostMapping("/campaign-templates/appCampaignGroupInfoWithCampaignGroupId/")
    @Timed
    public ResponseEntity<AppCampaignGroupInfo> getAppCampaignGroupInfoWithCampaignGroupId(@Valid @RequestBody AppCampaignGroupInfoWithCampaignGroupIdRequest appCampaignGroupInfoWithCampaignGroupIdRequest) {
        log.debug("REST request to getAppCampaignGroupInfoWithCampaignGroupId : {}", appCampaignGroupInfoWithCampaignGroupIdRequest.toString());
        CampaignGroup campaignGroup = campaignGroupService.findOne(appCampaignGroupInfoWithCampaignGroupIdRequest.getCampaignGroupId());
        Apps app = appsService.findOne(campaignGroup.getProjectId());
        AppCampaignGroupInfo appCampaignGroupInfo = new AppCampaignGroupInfo();
        appCampaignGroupInfo.setAppId(app.getId())
            .setAppName(app.getName())
...
//                            campaignTemplateService.delete(campaignTemplate.getId());
                            updateCampaignDeletionStatus(campaignTemplate.getId());
...
//                    return ResponseEntity.ok().headers(HeaderUtil.createAlert(campaignTemplate.getCampaignName() + " deleted", campaignTemplate.getId())).build();
    ...
    private void updateCampaignDeletionStatus(String campaignTemplateId){
        CampaignTemplate campaignTemplate = campaignTemplateService.findOne(campaignTemplateId);
        campaignTemplate.setStatus(Constants.CampaignTemplateStatus.DELETED.toString());
        campaignTemplate.setAlreadyDeleted(true);
        campaignTemplate.setDeletionTime(getCurrentDateTimeInUTCAsString());
        campaignTemplate.setStatus(Constants.CampaignTemplateStatus.DELETED.name());
        campaignTemplate.setLaunchEnabled(false);
        campaignTemplate.setEditEnabled(false);
        campaignTemplate.setCancelEnabled(false);
        campaignTemplate.setDeleteEnabled(false);
