...
export class FilterCriteriaComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.filterCriteriaService.query({
            page: this.page,
    ...
    trackId(index: number, item: FilterCriteria) {
        return item.id;
    }
    registerChangeInFilterCriteria() {
        this.eventSubscriber = this.eventManager.subscribe('filterCriteriaListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.filterCriteria.push(data[i]);
        }
    }
